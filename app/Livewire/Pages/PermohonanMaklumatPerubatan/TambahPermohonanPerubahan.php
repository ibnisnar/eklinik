...
class TambahPermohonanPerubahan extends Component
{
    public $permohonans;
    ...
    {
        $latestApplication = PermohonanMaklumatPerubatan::where('permohonan_type', 'perubahan')->orderBy('id', 'desc')->first();
        $year = date('y');
        $number = $latestApplication ? intval(substr($latestApplication->permohonan_no, 4)) + 1 : 1;
        $this->permohonan_no = 'PU' . $year . str_pad($number, 3, '0', STR_PAD_LEFT);
        $this->permohonan_type = 'perubahan';
    ...
    public function createPermohonanMaklumatPerubatan()
    {
        ...
        if ($permohonan) {
            ...
            $this->reset('rows');
            $this->rows = [['permohonan_perubatan' => '', 'permohonan_item' => '', 'permohonan_amaun' => '']];
            return redirect()->route('senarai-permohonan-perubahan-maklumat-perubatan.index')->with('message', 'Permohonan Maklumat Perubatan Berjaya DiHantar');
        } else {
            return redirect()->route('senarai-permohonan-perubahan-maklumat-perubatan.index')->with('message', 'Permohonan Maklumat Perubatan Tidak Berjaya DiHantar');
        }
    }

    public function render()
    {
        return view('livewire.pages.permohonan-maklumat-perubatan.tambah-permohonan-perubahan')->layout('layouts.app');
    }
}
